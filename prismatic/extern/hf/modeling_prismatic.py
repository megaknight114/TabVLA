...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def _process_action_masks(self, labels):
        ...
        # Handle cases where masks might be None (during inference)
        if current_action_mask is None or next_actions_mask is None:
            return None
    ...
    def forward(
        ...
        # === Handle Multimodal Forward ===
            ...
            if all_actions_mask is not None:
                language_embeddings = input_embeddings[~all_actions_mask].reshape(
                    input_embeddings.shape[0], -1, input_embeddings.shape[2]
                )  # (B, lang_seq_len, llm_dim)
            else:
                # During inference without labels, use all input embeddings as language embeddings
            ...
            elif all_actions_mask is not None:
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def set_num_images_in_input(self, num_images_in_input: int) -> None:
        """Set the number of images in the vision backbone input"""
        ...
        # Case 2: Through model attribute (LoRA wrapped)
        elif hasattr(self, 'model') and hasattr(self.model, 'vision_backbone'):
            vision_backbone = self.model.vision_backbone
        # Case 3: Through base_model attribute (PEFT wrapped)
        elif hasattr(self, 'base_model') and hasattr(self.base_model, 'vision_backbone'):
            vision_backbone = self.base_model.vision_backbone
        ...
        if vision_backbone is not None and hasattr(vision_backbone, 'set_num_images_in_input'):
            ...
            print(f"Warning: Could not find vision_backbone with set_num_images_in_input method")
